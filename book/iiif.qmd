...
- Implement the IIIF Presentation API which will create the all important IIIF Manifest files (there are many open source or vendor products that help handle this). These are the all important files which combine your images and structural data (like pagination or ordering) from your metadata source. They contain all the information that makes up your IIIF enabled object, from how to display the object, waht information to display (such as structure or the order of pages, or even where an illustration is located on a page, it can be as complex as you need!)
...
- Consider making your IIIF Manifests available publicly on your collection site so users can work them in all the interesting ways you've now enabled!
