...
Resource needs: Training models require a lot of computational resources.
...
Computer Vision is transforming the enrichment of library collections by automating the classification and annotation of visual materials. With the help of object detection and image recognition, it identifies objects, patterns, and relationships within images, enabling the creation of richer metadata and contextual information. This not only improves discoverability but also allows for deeper insights into the historical, artistic, or cultural significance of the materials. One example could be [Saint George on a Bike](https://saintgeorgeonabike.eu/) which was a project led by [Europeana](https://www.europeana.eu/) with the aim to improve the quality of cultural heritage objects' classification and metadata. Partners used CNNs for object detection to identify and create semantic context for the images.

Similarly, Computer Vision is improving access, delivery, and research within library collections by enabling more intuitive and sophisticated ways to explore and analyze visual data. Through developments in image recognition and retrieval, these technologies allow users to discover connections, find similar materials, and engage with collections in entirely new ways. 
[imgs.ai](https://imgs.ai/interface) is a text-based visual search engine for digital collections, using approximate k-NN algorithms and integrating the OpenAI CLIP model.
[Maken](https://www.nb.no/maken/) is a service developed by the National Library of Norway and it uses Computer Vision (object detection) to find similar images from the library's digital collection.
...
Semantic segmentation of maps - Computer Vision can be of use for identifying and categorising distinct elements within maps, such as roads, buildings, water bodies, and land use regions. Using CNNs and U-Net architectures, models partition maps into meaningful segments, allowing for detailed analysis and improved usability. This supports historical research, geographic analysis, and so on, by extracting structured data from complex cartographic sources.
[JADIS](https://bnf-jadis.github.io/#) is developed by the National Library of France, it is a tool for semantically segmented, georeferenced, and geocoded maps of Paris.
[MapReader](https://mapreader.readthedocs.io/en/v1.1.1/index.html) is an outcome of the [Living with the Machines project](https://livingwithmachines.ac.uk/). It's a Computer Vision pipeline for analysing large collections of historical maps.
